...
export const actions = {
    setAuthUserData: ( userId: number | null, email: string | null, login: string | null, isAuth: boolean ) => {
        return { type: '/auth/SET-USER-DATA', payload: { userId, email, login, isAuth } } as const
    },
    receivedErrMessage: ( error: string ) => {
        return { type: '/auth/ERROR-MESSAGE', error } as const
    },
    getCaptchaSuccess: ( captchaUrl: string ) => {
        return { type: '/auth/GET-CAPTCHA-URL', captchaUrl } as const
    }
